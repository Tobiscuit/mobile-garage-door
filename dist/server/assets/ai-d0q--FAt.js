import { g as getCloudflareContext, a as getDB, m as media, r as registerServerReference } from "./worker-entry-Cp_PwIlA.js";
import "node:events";
...
async function generateContent(systemPrompt, userPrompt, schema, responseMimeType = "application/json") {
  if (!apiKey) throw new Error("GEMINI_API_KEY is not set");
  try {
    const response = await genAI.models.generateContent({
      model: "gemini-2.0-flash",
      // Upgraded to faster/cheaper model for 2026
      contents: [
        {
          role: "user",
...
async function generateEmailDrafts(threadContext) {
...
async function generateProjectCaseStudy(prompt) {
  const schema = {
    type: Type.OBJECT,
    properties: {
...
async function generateMultimodalProjectCaseStudy(imagesContext, prompt, targetField, existingContext) {
  const fullSchema = {
    type: Type.OBJECT,
    properties: {
      title: { type: Type.STRING, description: "Catchy, professional title for the case study" },
...
Image Context:` });
    for (const img of imagesContext) {
      if (!img.url) continue;
      const fullUrl = img.url.startsWith("http") ? img.url : `${"https://garage-door.jrcodex.dev"}${img.url}`;
      try {
...
async function extractProjectContext(contextString) {
  const schema = {
    type: Type.OBJECT,
    properties: {
  ...
  try {
    const response = await genAI.models.generateContent({
      model: "gemini-3.1-pro-preview",
      // Pro model for complex extraction logic
      contents: [
        {
          role: "user",
          parts: [{ text: `${systemPrompt}
...
generatePostContent = /* @__PURE__ */ registerServerReference(generatePostContent, "8265110645c1", "generatePostContent");
generateEmailDrafts = /* @__PURE__ */ registerServerReference(generateEmailDrafts, "8265110645c1", "generateEmailDrafts");
generateProjectCaseStudy = /* @__PURE__ */ registerServerReference(generateProjectCaseStudy, "8265110645c1", "generateProjectCaseStudy");
generateMultimodalProjectCaseStudy = /* @__PURE__ */ registerServerReference(generateMultimodalProjectCaseStudy, "8265110645c1", "generateMultimodalProjectCaseStudy");
extractProjectContext = /* @__PURE__ */ registerServerReference(extractProjectContext, "8265110645c1", "extractProjectContext");
export {
  extractProjectContext,
  generateEmailDrafts,
  generateMultimodalProjectCaseStudy,
  generatePostContent,
