...
export function UserTable({ initialUsers }: { initialUsers: any[] }) {
  const columns = [
    ...
    },
    {
      ...
      cell: (user: any) => (
        <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase ${user.role === 'admin' ? 'bg-[#e74c3c]/20 text-[#e74c3c]' : 'bg-[#3498db]/20 text-[#3498db]'
          }`}>
          {user.role}
        </span>
      )
    },
    {
      ...
      cell: (user: any) => <span className="text-sm" style={{ color: 'var(--staff-muted)' }}>{user.createdAt ? new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).format(new Date(user.createdAt)) : '-'}</span>
    },
    {
      header: 'Actions',
      cell: (user: any) => (
